...
describe("architecture and release-governance hardening checks", () => {
  ...
  it("includes phase 4 live operations panel containers", async () => {
    const gatewayPanel = await read(
      "src/components/dashboard/GatewayBootstrap.astro",
    );
  ...
  it("uses SSR architecture with server-side gateway and API routes", async () => {
    ...
    const publicDashboardApi = await read(
      "src/pages/api/public/dashboard.json.ts",
    );
    const publicEventsApi = await read("src/pages/api/public/events.ts");
    const privateDashboardApi = await read(
      "src/pages/api/private/dashboard.json.ts",
    );
    ...
    const deprecatedDashboardApi = await read(
      "src/pages/api/dashboard.json.ts",
    );
  ...
  it("injects theme vars style and data-cs-theme attribute in Base layout", async () => {
    ...
    expect(base).toContain(
      "import { resolveTheme, listThemes } from '../config/themes'",
    );
