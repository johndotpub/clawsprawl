  ...
  health: {
    ...
    channels: {
      slack: {
        running: true,
        probeOk: true,
        botUsername: "secret-bot",
        lastError: "bad things",
      },
    },
  },
  status: {
    ...
    heartbeat: {
      defaultAgentId: "ceo",
      agents: [
        { agentId: "ceo", enabled: true, every: "10m", everyMs: 600000 },
      ],
    },
  },
...
describe("public/private dashboard shaping", () => {
  it("removes private top-level fields from the public snapshot", () => {
    ...
    expect(publicSnapshot.cronScheduler).toEqual({
      enabled: true,
      jobs: 2,
      nextWakeAtMs: 123,
    });
    expect(publicSnapshot.health?.channels?.slack).toEqual({
      running: true,
      probeOk: true,
    });
    expect(
      publicSnapshot.channelsStatus?.channelAccounts?.slack?.[0],
    ).toMatchObject({
      connected: true,
  ...
  it("never exposes raw gateway events publicly", () => {
    ...
    ).toMatchObject({ event: "session.tool" });
  });

  it("keeps progress cards and active run ids private (gateway >= 2026.9)", () => {
    const withCards = {
      ...snapshot,
      progressCards: {
        "agent:ceo:main": {
      ...
      activeRunIds: ["agent:ceo:main"],
    } as unknown as DashboardSnapshot;

    const publicSnapshot = buildPublicSnapshot(withCards);
    // Progress cards carry session keys + live agent activity — private-only.
    expect(publicSnapshot.progressCards).toEqual({});
    expect(publicSnapshot.activeRunIds).toEqual([]);
    expect(JSON.stringify(publicSnapshot)).not.toContain("agent:ceo:main");
    expect(JSON.stringify(publicSnapshot)).not.toContain("Migration work");
  });
  ...
  it("preserves public-safe status session and heartbeat summaries", () => {
    ...
    const publicSnapshot = buildPublicSnapshot(
      enriched as unknown as DashboardSnapshot,
    );
    expect(publicSnapshot.agents).toEqual([
      { id: "ceo", name: "Chief", model: "openai/gpt-5" },
    ]);
  ...
  it("handles missing optional sections by returning null summaries", () => {
    ...
    const publicSnapshot = buildPublicSnapshot(
      minimal as unknown as DashboardSnapshot,
    );
  ...
  it("falls back safely when health/status nested shapes are malformed", () => {
    ...
    const publicSnapshot = buildPublicSnapshot(
      malformed as unknown as DashboardSnapshot,
    );
