...
export function renderAgentRows(
  ...
  const rows = state.agents.map((agent) => {
    ...
    return row(
      [label(agentId), muted(`sessions:${count}`), tag(model)],
      "stack",
    );
  });
...
export function renderCronRows(state: DashboardState): string {
  ...
  const rows = state.cronJobs.map((job) => {
    ...
    const schedule =
      typeof job.schedule === "object" && job.schedule !== null
        ? // SAFETY: the union narrowing above proves schedule is CronSchedule | string
          ...
          asString(
            (job.schedule as unknown as Record<string, unknown>).expr,
            "n/a",
          )
    ...
    const runStatus = run_
      ? asString(run_.status, enabled ? "pending" : "disabled")
      : enabled
        ? "pending"
        : "disabled";
    const errorDetail = extractCronRunError(run_);
    const tone: "ok" | "error" | "warn" = runStatus
      .toLowerCase()
      .includes("error")
      ? "error"
      : runStatus === "ok"
    ...
    return rowWithDetail(
      [label(name), muted(schedule), badge(tone, runStatus)],
      detail,
    );
  });
...
export function renderProviderRows(state: DashboardState): string {
  ...
  const rows = summary.map((entry) => {
    ...
    return row([
      ...
      tag(`models:${entry.modelCount}`),
    ]);
  });
  return rows.length > 0
    ? rows.join("")
    : emptyRow("No model provider data loaded.");
}
...
export function renderSessionRows(state: DashboardState): string {
  ...
  const rows = sessions.map((session) => {
    ...
    const age =
      typeof session.updatedAt === "number" ? formatAge(session.updatedAt) : "";
  ...
  const overflow =
    state.sessions.length > SESSION_DISPLAY_LIMIT
...
export function renderHealthRows(state: DashboardState): string {
  ...
  const durationSuffix =
    typeof state.health.durationMs === "number"
      ? ` (${state.health.durationMs}ms)`
      : "";
  parts.push(
    row([
      label("Overall"),
      badge(overallTone, overallText),
      muted(durationSuffix),
    ]),
  );
...
export function renderStatusRows(state: DashboardState): string {
  ...
  if (s.tasks) {
    const t = s.tasks;
    const failCls =
      t.failures > 0 ? "text-terminal-error" : "text-terminal-muted";
    parts.push(
      row([
        label("Tasks"),
        muted(`total:${t.total}`),
        '<span class="text-terminal-green">' +
          escapeHtml(`active:${t.active}`) +
          "</span>",
  ...
  if (s.taskAudit && (s.taskAudit.warnings > 0 || s.taskAudit.errors > 0)) {
    const auditCls =
      s.taskAudit.errors > 0 ? "text-terminal-error" : "text-terminal-amber";
    parts.push(
      row([
        label("Audit"),
  ...
  if (s.sessions?.byAgent && s.sessions.byAgent.length > 0) {
    const agentParts = s.sessions.byAgent
      .map((a) => `${a.agentId}:${a.count}`)
      .join(" ");
    parts.push(
      row([
        label("Sessions"),
        muted(`total:${s.sessions.count}`),
        tag(agentParts),
      ]),
    );
  }
...
export function renderSkillsRows(state: DashboardState): string {
  ...
  const totalEligible = state.skillsStatus.skills.filter(
    (s) => s.eligible,
  ).length;
  const totalSkills = state.skillsStatus.skills.length;
  const summary = row([
    muted("Skills"),
    tag(`${totalEligible}/${totalSkills} eligible`),
  ]);
...
export function renderChannelsStatusRows(state: DashboardState): string {
  ...
  for (const channel of state.channelsStatus.channelOrder) {
    ...
    for (const acc of accounts) {
      const connBadge = acc.connected
        ? badge("ok", "connected")
        : badge("error", "disconnected");
      const botName = acc.bot ? tag(`bot:${acc.bot.username}`) : "";
      const lastIn = acc.lastInboundAt
        ? muted(`in:${formatAge(acc.lastInboundAt)}`)
        : "";
      const lastOut = acc.lastOutboundAt
        ? muted(`out:${formatAge(acc.lastOutboundAt)}`)
        : "";
      const retries =
        acc.reconnectAttempts > 0
          ? muted(`retries:${acc.reconnectAttempts}`)
          : "";
      const children = [
        label(label_),
        connBadge,
      ...
      parts.push(
        errorLine ? rowWithDetail(children, errorLine) : row(children),
      );
    }
  }
  ...
  return parts.length > 0
    ? parts.join("")
    : emptyRow("No channel status available.");
}
...
export function renderCronSchedulerRows(state: DashboardState): string {
  ...
  const enabledBadge = cs.enabled
    ? badge("ok", "enabled")
    : badge("error", "disabled");
  const nextWake = cs.nextWakeAtMs
    ? muted(`next:${formatAge(cs.nextWakeAtMs)}`)
    : muted("no next wake");
...
export function renderMemoryStatusRows(state: DashboardState): string {
  ...
  const embeddingBadge = ms.embedding.ok
    ? badge("ok", "ok")
    : badge("error", "fail");
  parts.push(
    row([
      label("Embedding"),
      embeddingBadge,
      muted(`agent:${ms.agentId}`),
      muted(`provider:${ms.provider}`),
    ]),
  );

  const dreamBadge = ms.dreaming.enabled
    ? badge("ok", "enabled")
    : badge("muted", "disabled");
  parts.push(
    row([
      label("Dreaming"),
      dreamBadge,
  ...
  for (const [name, phase] of Object.entries(ms.dreaming.phases)) {
    const phaseBadge = phase.enabled
      ? badge("ok", "on")
      : badge("muted", "off");
...
export function renderPermissionActivityRows(state: DashboardState): string {
  ...
  return events
    .map((event) => {
      ...
      const tone: "ok" | "warn" | "muted" = isRequested
        ? "warn"
        : result === "granted"
          ? "ok"
          : "muted";
...
export function renderToolExecutionRows(state: DashboardState): string {
  ...
  return events
    .map((event) => {
      ...
      const tool = asString(
        payload.tool,
        asString(payload.toolName, asString(payload.name, "")),
      );
      const durationMs =
      ...
      const tone: "ok" | "error" | "warn" = isRunning
        ? "warn"
        : success && !error
          ? "ok"
          : "error";
      ...
      if (error) {
        return rowWithDetail(
          parts,
          `<p class="mt-1 text-terminal-error">${escapeHtml(truncate(error, 80))}</p>`,
        );
      }
      return row(parts);
    })
...
export function renderFileTrackingRows(state: DashboardState): string {
  ...
  if (state.fileStatus && state.fileStatus.length > 0) {
    parts.push(
      row([
        muted("📁 Modified files"),
        tag(`${state.fileStatus.length} files`),
      ]),
    );
    for (const file of state.fileStatus.slice(0, 20)) {
      const statusTone: "ok" | "warn" | "error" | "muted" =
  ...
  const fileEvents = filterEvents(state.events, (name) =>
    name.startsWith("file."),
  );

  if (fileEvents.length > 0) {
    parts.push(
      row([muted("📝 Recent file events"), tag(`${fileEvents.length} events`)]),
    );
  ...
  return parts.length > 0
    ? parts.join("")
    : emptyRow("No file changes tracked yet.");
}
...
export function renderProgressCardRows(
  ...
  const rows = entries.map((card) => {
    ...
    const bar = steps
      ...
      .join("");
    const barText =
      steps.length > 0 ? `[${bar}] ${done}/${steps.length}` : "[—] 0/0";
...
export function renderSessionDetailRows(state: DashboardState): string {
  ...
  const rows = state.sessionDetails
    ...
    .map((session) => {
      ...
      if (typeof session.messageCount === "number")
        parts.push(muted(`msgs:${session.messageCount}`));
      if (typeof session.tokenCount === "number")
        parts.push(muted(`tokens:${session.tokenCount}`));
      if (typeof session.lastActivityAt === "number")
        parts.push(muted(formatAge(session.lastActivityAt)));
      return row(parts, "stack");
    });

  const overflow =
    state.sessionDetails.length > SESSION_DISPLAY_LIMIT
      ? emptyRow(
          `… and ${state.sessionDetails.length - SESSION_DISPLAY_LIMIT} more`,
        )
      : "";
  return rows.join("") + overflow;
}
...
// ---------------------------------------------------------------------------
// Phase 3 read-only panels (gateway ≥ 2026.9, method-gated)
// ---------------------------------------------------------------------------

/** Render Task Ledger rows from `tasks.list`. */
export function renderTaskLedgerRows(tasks: TaskLedgerEntry[] | null): string {
  if (!tasks) return emptyRow("No task data loaded.");
  if (tasks.length === 0) return emptyRow("No active tasks.");

  const rows = tasks.slice(0, SESSION_DISPLAY_LIMIT).map((task) => {
    const tone: "ok" | "error" | "warn" | "muted" =
      task.status === "running"
        ? "ok"
        : task.status === "blocked" || task.status === "failed"
          ...
          : task.status === "pending" || task.status === "queued"
            ...
            : "muted";
    const parts: string[] = [
      label(asString(task.name, asString(task.title, task.id))),
    ];
    parts.push(badge(tone, task.status));
    if (task.priority) parts.push(muted(task.priority));
    return row(parts, "stack");
  });
  const overflow =
    tasks.length > SESSION_DISPLAY_LIMIT
      ? emptyRow(`… and ${tasks.length - SESSION_DISPLAY_LIMIT} more`)
      : "";
  return rows.join("") + overflow;
}

/** Render Session Usage Timeseries rows from `sessions.usage.timeseries`. */
export function renderUsageTimeseriesRows(
  buckets: UsageTimeseriesEntry[] | null,
): string {
  if (!buckets) return emptyRow("No usage timeseries data loaded.");
  if (buckets.length === 0) return emptyRow("No usage buckets recorded.");

  const recent = buckets.slice(-30);
  const rows = recent.map((bucket) => {
    const parts: string[] = [tag(bucket.ts)];
    if (typeof bucket.tokens === "number")
      parts.push(muted(`tokens: ${formatTokenCount(bucket.tokens)}`));
    if (typeof bucket.cost === "number")
      parts.push(muted(`$${bucket.cost.toFixed(4)}`));
    return row(parts, "stack");
  });
  const overflow =
    buckets.length > 30
      ? emptyRow(`… oldest ${buckets.length - 30} buckets hidden`)
      : "";
  return rows.join("") + overflow;
}

/** Render Node Fleet rows from `node.list`. */
export function renderNodeFleetRows(nodes: NodeFleetEntry[] | null): string {
  if (!nodes) return emptyRow("No node fleet data loaded.");
  if (nodes.length === 0) return emptyRow("No paired nodes.");

  const rows = nodes.slice(0, SESSION_DISPLAY_LIMIT).map((node) => {
    const name = asString(node.name, node.nodeId);
    const parts: string[] = [label(name)];
    if (node.platform) parts.push(tag(node.platform));
    if (node.version) parts.push(muted(node.version));
    parts.push(
      badge(
        node.connected ? "ok" : "muted",
        node.connected ? "connected" : "offline",
      ),
    );
    if (node.lastSeenAtMs) parts.push(muted(formatAge(node.lastSeenAtMs)));
    const detail = node.hostStats
      ? `<div class="text-terminal-muted">mem: ${formatBytesToGb(node.hostStats.memoryFreeBytes)} free of ${formatBytesToGb(node.hostStats.memoryTotalBytes)} · cpus: ${asString(node.hostStats.cpuCount, "n/a")}</div>`
      : "";
    return detail ? rowWithDetail(parts, detail) : row(parts, "stack");
  });
  const overflow =
    nodes.length > SESSION_DISPLAY_LIMIT
      ? emptyRow(`… and ${nodes.length - SESSION_DISPLAY_LIMIT} more`)
      : "";
  return rows.join("") + overflow;
}

/** Render Gateway Stability rows from `diagnostics.stability`. */
export function renderStabilityRows(
  entries: StabilityEntry[] | null | undefined,
): string {
  if (entries === undefined) return emptyRow("Stability data unavailable.");
  if (!entries || entries.length === 0)
    return emptyRow("No stability events recorded.");

  const recent = entries.slice(-20);
  const rows = recent.map((entry) => {
    const parts: string[] = [tag(entry.ts), label(entry.kind)];
    if (entry.summary) parts.push(muted(truncate(entry.summary, 70)));
    return row(parts, "stack");
  });
  const overflow =
    entries.length > 20
      ? emptyRow(`… oldest ${entries.length - 20} entries hidden`)
      : "";
  return rows.join("") + overflow;
}

/** Render Audit Timeline rows from `audit.activity.list`. */
export function renderAuditTimelineRows(
  entries: AuditTimelineEntry[] | null,
): string {
  if (!entries) return emptyRow("No audit data loaded.");
  if (entries.length === 0) return emptyRow("No audit activity recorded.");

  const rows = entries.slice(0, SESSION_DISPLAY_LIMIT).map((entry) => {
    const parts: string[] = [tag(entry.ts)];
    if (entry.actor) parts.push(tag(entry.actor));
    parts.push(label(entry.action));
    if (entry.outcome)
      parts.push(
        badge(
          entry.outcome === "ok" || entry.outcome === "success"
            ? "ok"
            : entry.outcome === "error" || entry.outcome === "denied"
              ? "error"
              : "muted",
          entry.outcome,
        ),
      );
    const target = entry.target ? muted(truncate(entry.target, 50)) : "";
    return target
      ? rowWithDetail(parts, `<div class="text-terminal-muted">${target}</div>`)
      : row(parts, "stack");
  });
  const overflow =
    entries.length > SESSION_DISPLAY_LIMIT
      ? emptyRow(`… and ${entries.length - SESSION_DISPLAY_LIMIT} more`)
      : "";
  return rows.join("") + overflow;
}

/** Render Config Schema rows from `config.schema` (alphabetical, capped). */
export function renderConfigSchemaRows(
  schema: Record<string, ConfigSchemaEntry> | null,
): string {
  if (!schema) return emptyRow("No config schema data loaded.");
  const keys = Object.keys(schema).sort();
  if (keys.length === 0) return emptyRow("Config schema is empty.");

  const shown = keys.slice(0, 50);
  const rows = shown.map((key) => {
    const entry = schema[key];
    const parts: string[] = [tag(key)];
    if (entry.type) parts.push(muted(entry.type));
    if (entry.description) parts.push(muted(truncate(entry.description, 80)));
    return row(parts, "stack");
  });
  const overflow =
    keys.length > 50 ? emptyRow(`… and ${keys.length - 50} more`) : "";
  return rows.join("") + overflow;
}

/** Render Skill Proposal queue rows from `skills.proposals.list`. */
export function renderSkillProposalRows(
  proposals: SkillProposalEntry[] | null,
): string {
  if (!proposals) return emptyRow("No skill proposals loaded.");
  if (proposals.length === 0) return emptyRow("No pending skill proposals.");

  const rows = proposals.slice(0, SESSION_DISPLAY_LIMIT).map((proposal) => {
    const tone: "ok" | "error" | "warn" | "muted" =
      proposal.status === "approved"
        ...
        : proposal.status === "rejected"
          ...
          : proposal.status === "pending"
            ? "warn"
            : "muted";
    const parts: string[] = [label(asString(proposal.id, "proposal"))];
    parts.push(badge(tone, proposal.status));
    if (proposal.verdict) parts.push(muted(`verdict: ${proposal.verdict}`));
    if (proposal.author) parts.push(muted(proposal.author));
    return row(parts, "stack");
  });
  const overflow =
    proposals.length > SESSION_DISPLAY_LIMIT
      ? emptyRow(`… and ${proposals.length - SESSION_DISPLAY_LIMIT} more`)
      : "";
  return rows.join("") + overflow;
}
